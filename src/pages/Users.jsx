...
export const Users = () => {
  ...
  const {
    ...
    isLoading,
    isFetching,
    refetch: fetchUpdatedUsers,
  } = useGetUsersQuery(debounceSearchFilters, {
  ...
  const [
    ...
    { isLoading: isActivateOrDeActivateUserLoading },
  ] = useActivateOrDeActivateUserMutation();

  const [unLockUser, { isLoading: isUnlockUserLoading }] =
    useUnLockUserMutation();
  ...
  const handleUserOpenMenu = (e, user) => {
  ...
  const activateOrDeActivateSelectedUser = async () => {
    handleUserCloseMenu();

    try {
      const response = await activateOrDeActivateUser(selectedUser.id).unwrap();
  ...
  const unLockSelectedUser = async () => {
    ...
    try {
      const response = await unLockUser(selectedUser.id).unwrap();
      useSuccessRef.current.setSuccessMsg(response.message, "");
      useSuccessRef.current.openModal();
      fetchUpdatedUsers();
    } catch (e) {
  ...
  return (
    <>
      {(isLoading ||
        isFetching ||
        isActivateOrDeActivateUserLoading ||
        isUnlockUserLoading) && <Loading />}
      ...
      <Table component={Paper}>
        <Table>
          ...
          >
            <MenuItem
              onClick={activateOrDeActivateSelectedUser}
              sx={{ textAlign: "center" }}
            >
              <Typography variant="body1">
                {selectedUser?.active ? "De-Activate User" : "Activate User"}
              </Typography>
            </MenuItem>
            <MenuItem onClick={unLockSelectedUser} sx={{ textAlign: "center" }}>
              <Typography variant="body1">Un Lock User</Typography>
            </MenuItem>
          </Menu>
          <TableFooter>
            <TableRow>
